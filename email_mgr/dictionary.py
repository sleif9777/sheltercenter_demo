...
def get_appt_replacements(appt):
    appt_replacements = {'*APT_DOG*': appt.dog}
...
def get_signature():
    try:
        return request.user.profile.signature
    except:
...
def get_watchlist_replacements(adopter, date):
    ...
    return statuses + foster_host_disclaimer + first_come_first_serve_disclaimer
    ...
    for item in replacements_master:
        if item:
            for key in replacements_master[item]:
                if key in html:
                    print(key, replacements_master[item][key])
